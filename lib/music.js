...
function initialize(botObj,voiceChannel,textChannel) {
	...
	for (var i in bot.servers[0].channels) {
		...
		if (bot.servers[0].channels[i].name == textChannel && bot.servers[0].channels[i].type == "text") {
			...
			bot.sendMessage(musicStatusChannel,"testing");
		}
		//if matches voice
...
var checkQueue = function() {
	...
	if ((fifo.first() == null) && voiceConnection == undefined) {
		currentSong = "None";
	}
...
function playNextSong() {
	...
	bot.sendMessage(musicStatusChannel,"Now Playing: " + currentSong.title);
...
function addToPlayList(data) {
	if (data == null) {
		...
		for (var node = fifo.node; node; node = fifo.next(node)) {
			if (node.value.title != undefined)
				msgResponse += node.value.title + ",";
		}
